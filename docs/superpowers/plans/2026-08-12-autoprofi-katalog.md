...
**Сборка идёт в один процесс.** По умолчанию Next.js поднимает воркер на каждое ядро, и все они одновременно открывают одну файловую базу PGlite. Это однопоточный WebAssembly, конкурентного доступа он не держит — сборка падает с `RuntimeError: Aborted()`. Лечится строкой `experimental: { cpus: 1 }` в `next.config.ts`. Генерация 8 238 страниц занимает около двух с половиной минут вместо примерно сорока секунд — приемлемо, потому что это статика и собирается она раз в выкладку.

**Режим разработки с файловой базой не работает.** `npm run dev` падает на открытии PGlite с `RuntimeError: Aborted()`. Сборка и `npm run start` работают, поэтому смотреть сайт нужно так:

```bash
npm run build && npm run start
```

Причина не найдена. Известно, что дело не в бандлинге (`serverExternalPackages` уже настроен), не в относительном пути и не в перезагрузке модулей — все три версии проверены. Отложено: на статическом сайте dev-сервер нужен только для правки стилей, а страницы всё равно генерируются на сборке.

**База ломается от аварийных остановок.** PostgreSQL держит в каталоге файл `postmaster.pid`. Процесс, ушедший без закрытия, оставляет замок, и следующая попытка открыть базу падает — причём с тем же невнятным `Aborted()`, что и предыдущая проблема. Дважды это увело диагностику не туда.

Лечится двумя вещами: `scripts/import.ts` закрывает базу через `client.close()`, а если замок всё же остался — `rm .pgdata/postmaster.pid` и при необходимости `npm run import`, который пересоздаёт каталог начисто.

**Общее число товаров нельзя складывать из счётчиков по маркам.** Один фаркоп подходит к машинам разных марок и попадает в сумму несколько раз: на главной выходило 7 339 вместо 5 808. Для этого есть `countProducts()`.
